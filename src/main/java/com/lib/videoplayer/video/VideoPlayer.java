...
public class VideoPlayer extends FrameLayout
    ...
    public void restart() {
        if (mCurrentState == STATE_PAUSED) {
            ...
            if(mCurrentState == STATE_COMPLETED){ Util.savePlayPosition(mContext, mUrl, 0);}
            openMediaPlayer();
        } else {
...
//
                    ((IjkMediaPlayer) mMediaPlayer).native_setLogLevel(IjkMediaPlayer.IJK_LOG_DEBUG);
                    //HTTP和HTTPS 视频同域名时，清除dns缓存
                    ...
                    //0是开启了环路滤波，48基本没启用环路滤波 所以清晰度更低，但是解码性能开销小
                    ...
                    //播放重连次数
                    ((IjkMediaPlayer)mMediaPlayer).setOption(IjkMediaPlayer.OPT_CATEGORY_PLAYER,"reconnect",3);
                    break;
            }
